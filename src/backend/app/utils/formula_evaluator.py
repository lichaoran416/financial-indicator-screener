...
class FormulaEvaluator:
    ...
    def get_time_series_value(
        ...
        if isinstance(value, dict):
            if isinstance(year, tuple):
                ...
                if year not in value or value[year] is None:
                    raise FormulaEvaluatorError(
                        f"No data for {metric_name}[{year}]"
                    )
        ...
        raise FormulaEvaluatorError(
            f"Invalid time series value type for {metric_name}"
        )
    ...
    def evaluate_list(
        ...
        if node.node_type in (ASTNodeType.BINARY_OP, ASTNodeType.UNARY_OP):
            ...
            results = []
            max_len = max(len(left_values) if left_values else 0, len(right_values) if right_values else 0, 1)
